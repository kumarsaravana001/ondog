class CategoryWishShow {
  bool? status;
  List<CategoryWiseListData>? data;

  CategoryWishShow({this.status, this.data});

  CategoryWishShow.fromJson(Map<String, dynamic> json) {
    status = json['status'];
    if (json['data'] != null) {
      data = <CategoryWiseListData>[];
      json['data'].forEach((v) {
        data!.add(CategoryWiseListData.fromJson(v));
      });
    }
  }
...
class CategoryWiseListData {
  ...
  CategoryWiseListData.fromJson(Map<String, dynamic> json) {
    showId = json['show_id'];
